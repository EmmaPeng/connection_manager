...
ngx_xmpp_proxy_xmlstart(void *data, const char *el, const char **attr)
{
...
#if (NGX_DEBUG)
	for (i = 0; i < pctx->xml_depth; i++)
		printf("\t");

	printf("%lu: %s",pctx->xml_depth, el);
	...
	if(pctx->xml_depth==1){
		pctx->xml_pre_index=XML_GetCurrentByteIndex(parser);
		if(ngx_memcmp("stream:stream",el,13) == 0 && pctx->xml_state == XMPP_SESSION_CREATED){
			//XML_ParserReset(pctx->xmlparser,"UTF-8");
			b = pctx->wbuffer;
			size = XML_CM2C_FEATURES_BIND_LEN + pctx->channel_id.len - 2;
			...
			sprintf(b->last,XML_CM2C_FEATURES_BIND,pctx->channel_id.data);
			b->last += size;
	...
	}else if(pctx->xml_depth==0){
		if(ngx_memcmp("stream:stream",el,13) == 0){
			
			if(pctx->xml_state == XMPP_SESSION_UNUSE){

				pctx->xml_state = XMPP_SESSION_CREATED;
				size = XML_CM2S_CREATE_SESSION_LEN + ctx->xml_tofrom.len + pctx->channel_id.len * 2 + s->connection->addr_text.len *2 - 10;
				if(b->end - b->last < size)
				...
				sprintf((char*)b->last, XML_CM2S_CREATE_SESSION, (char*)ctx->xml_tofrom.data, (char*)pctx->channel_id.data, (char*)pctx->channel_id.data, (char*)s->connection->addr_text.data, (char*)s->connection->addr_text.data);
				//ngx_copy(b->last,XML_CM2S_CREATE_SESSION,XML_CM2S_CREATE_SESSION_LEN);
				b->last+=size;
				//ngx_copy(b->last,pctx->channel_id.data,pctx->channel_id.len);
				if(ngx_tcp_xmpp_upstream_send(ctx,NULL,0) == NGX_ERROR){
					XML_StopParser(parser,0);
					pctx->xml_state = XMPP_SESSION_ERROR;
				}
...
ngx_xmpp_proxy_xmlend(void *data, const char *el)
{
	...
	if(pctx->xml_depth==1){
		...
		// 33 => 31+2
		if(b->end - b->last < (33+pctx->channel_id.len) && ngx_tcp_xmpp_upstream_send(ctx,NULL,0) == NGX_ERROR){
			XML_StopParser(parser,0);
			pctx->xml_state = XMPP_SESSION_ERROR;
			return;
		}
		ngx_copy(b->last,(u_char*)"<route to=\"etop.com\" streamid=\"",31);
		b->last += 31;
		ngx_copy(b->last,pctx->channel_id.data,pctx->channel_id.len);
		b->last += pctx->channel_id.len;
		
		ngx_copy(b->last,(u_char*)"\" from=\"",8);
		b->last += 8;
		ngx_copy(b->last,ctx->xml_tofrom.data,ctx->xml_tofrom.len);
		b->last += ctx->xml_tofrom.len;
		...
		size = index - pctx->xml_pre_index + strlen(el)+3;
		if(b->end - b->last < size && ngx_tcp_xmpp_upstream_send(ctx,NULL,0) == NGX_ERROR){
			XML_StopParser(parser,0);
			pctx->xml_state = XMPP_SESSION_ERROR;
			return;
		}
		b->last = ngx_copy(b->last,str + (offset-(index - pctx->xml_pre_index)),size);
...
//	printf("pre-offset: %d, offset: %d, size: %d, xml_depth: %d, %s, %s\n",preOffset, offset,size ,ctx->xml_depth, sbuf,el);
		...
		pctx->xml_state = XMPP_SESSION_CLOSE;
	}
}
...
ngx_xmpp_upstream_xmlstart(void *data, const char *el, const char **attr)
{
	...
	if(ctx->state == XMPP_CM_READY){
			...
			switch(ctx->xmpp_type){
				case XMPP_IQ_RESULT:
					if(ngx_memcmp("session",el,7) == 0){
						...
						ctx->xmpp_type = XMPP_IQ_RESULT_SESSION;
					}
					break;
		...
		}else if(ctx->xml_depth == 3 && ctx->xmpp_type == XMPP_IQ_RESULT_SESSION && ngx_memcmp("create",el,6) == 0){
			ctx->xmpp_type = XMPP_IQ_RESULT_SESSION_CREATE;
		}else if(ctx->xml_depth == 3 && ctx->xmpp_type == XMPP_IQ_RESULT_SESSION && ngx_memcmp("close",el,5) == 0){
			ctx->xmpp_type = XMPP_IQ_RESULT_SESSION_CLOSE;
		}else if(ctx->xml_depth > 2){
			switch(ctx->xmpp_type){
				case XMPP_ROUTE:{
					int 	offset,size,index;
			...
			switch(ctx->xmpp_type){
				case XMPP_ROUTE:
					...
					if(s){
						...
						//pctx = ngx_tcp_get_module_ctx(s, ngx_xmpp_proxy_module);
						//b = pctx->wbuffer;
						if(ngx_tcp_xmpp_proxy_send(s->connection,str+(offset - size),size) == NGX_ERROR){
							XML_StopParser(parser,0);
							return;
						}
						ctx->xml_pre_index = index;
					...
					break;
				case XMPP_IQ_RESULT_SESSION_CLOSE:
				case XMPP_IQ_RESULT_SESSION_CREATE:
					//if()
					//break;
				case XMPP_IQ_RESULT_SESSION:
				case XMPP_IQ_RESULT:
