...
ngx_xmpp_proxy_init_session(ngx_tcp_session_t *s) 
{
	...
	s->wbuffer = ngx_create_temp_buf(s->connection->pool, pcf->buffer_size);
    if (s->wbuffer == NULL) {
	...
	s->channel_id.data = (u_char*)ngx_pcalloc(s->connection->pool,XMPP_MAX_CHN_LEN);
	ngx_time_update();
	ngx_sprintf(s->channel_id.data,"%d-%d",ngx_time(),s->connection->fd);
	s->channel_id.len=ngx_strlen(s->channel_id.data);
	
	
	ngx_hashtable_set(&CS_HT,s->channel_id.data,s->channel_id.len,s);
	
	s->xmlparser = XML_ParserCreate(NULL);
	if (s->xmlparser == NULL) {
	...
	s->xml_depth=0;
	XML_SetUserData(s->xmlparser, s);
	XML_SetElementHandler(s->xmlparser, ngx_xmpp_proxy_xmlstart, ngx_xmpp_proxy_xmlend);
		...
		if (n > 0)
			...
			b->last[0] = '\0';
						...
						if (n > 0) {
							...
							if (XML_Parse(s->xmlparser, b->pos, n, 0) == XML_STATUS_ERROR) {
								int errCode=XML_GetErrorCode(s->xmlparser);
								if(errCode==36){
									ngx_log_debug2(NGX_LOG_DEBUG_TCP, ev->log, 0,
												   "xml parser error(%d): %s",XML_GetErrorCode(s->xmlparser),
												   XML_ErrorString(XML_GetErrorCode(s->xmlparser)));
								}else{
									ngx_log_debug3(NGX_LOG_DEBUG_TCP, ev->log, 0,
												   "Parse error at line %lu:(%d) %s",XML_GetCurrentLineNumber(s->xmlparser),XML_GetErrorCode(s->xmlparser),
												   XML_ErrorString(XML_GetErrorCode(s->xmlparser)));
								}
								ngx_tcp_finalize_session(s);
								return;
							}
							if (s->xml_state == XMPP_SESSION_CLOSE){
