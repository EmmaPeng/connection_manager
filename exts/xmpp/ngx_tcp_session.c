...
ngx_tcp_init_session(ngx_connection_t *c)
{
    ...
    s->pool = c->pool;
	s->xml_state = XMPP_SESSION_UNUSE;
	
	s->connection->addr_text.data[s->connection->addr_text.len] = '\0';
...
ngx_tcp_close_connection(ngx_connection_t *c)
{
	...
	if(s->xmlparser)XML_ParserFree(s->xmlparser);
	ngx_hashtable_remove(CS_HT,s->channel_id.data,s->channel_id.len);
